...
; Copy 64 bytes using ymm0 and ymm1 then return
...
; Here size can only be 8-15 bytes
; Copy the first quadword and the last quadword of src into dest
...
; Here size can only be greater than 64 bytes
; Check is size > __x86_rep_movsb_threshold
...
; Here size can only be 129-256 bytes
; Copy 8 * 32 = 256 bytes using ymm0-ymm7 registers
...
<+305>:	vzeroupper
<+308>:	ret
; Here size can only be 257-__x86_rep_movsb_threshold bytes 
; Check is dest is a pointer into the src buffer (ie. writing to dest overwrite src)
<+309>:	mov    rcx,rdi
<+312>:	sub    rcx,rsi
<+315>:	cmp    rcx,rdx
<+318>:	jb     0x7ffff7f272f0 <__memmove_avx_unaligned_erms+496>
; Check if size > __x86_shared_non_temporal_threshold 
...
; Check if size >= __x86_rep_movsb_stop_threshold
...
; Check is size > src - dest (ie writing in the end of dest will overwrite the start of src)
<+816>:	neg    rcx
<+819>:	cmp    rdx,rcx
<+822>:	ja     0x7ffff7f2726b <__memmove_avx_unaligned_erms+363>
; Copy 64 bytes of src to dest using ymm0 and ymm1
; Then adjust rsi and rdi by adding the offset to the next 64 boundary for rdi
; Then ajust the remaining byte count to account for the offset
...
<+856>:	sub    rdi,r8
<+859>:	add    rdx,r8
<+862>:	not    ecx				; ecx = src - dest - 1
<+864>:	mov    r10,rdx
<+867>:	test   ecx,0xf00
<+873>:	je     0x7ffff7f275f0 <__memmove_avx_unaligned_erms+1264>
; Check if adjusted size >= __x86_shared_non_temporal_threshold / 16
<+879>:	shl    r11,0x4
<+883>:	cmp    rdx,r11
...
<+912>:	mov    ecx,0x20
; Copy 512 bytes using ymm0-ymm7 and prefetch the next 8 * 64 = 512 bytes
...
; Repeat while there are blocks of 2^13 bytes left
<+1091>:	add    rdi,0x1000
<+1098>:	add    rsi,0x1000
<+1105>:	dec    r10
<+1108>:	jne    0x7ffff7f27490 <__memmove_avx_unaligned_erms+912>
; Here there is less than 2^13 left
; Check if adjusted size % (2^13) <= 128
<+1114>:	sfence
<+1117>:	cmp    edx,0x80
<+1123>:	jbe    0x7ffff7f275ba <__memmove_avx_unaligned_erms+1210>
; While adjusted size % (2^64) > 128 prefetch and copy blocks of 4 * 32 = 128 bytes
...
; Copy one block of 4 * 32 = 128 bytes
...
<+1261>:	ret
<+1262>:	xchg   ax,ax
; Compute adjusted size % (2^14)
<+1264>:	and    edx,0x3fff
<+1270>:	shr    r10,0xe 		; divide adjusted size by 2^14
<+1274>:	nop    WORD PTR [rax+rax*1+0x0]
; Repeat this loop 32 times to copy a total of 2^14 bytes
<+1280>:	mov    ecx,0x20
; Copy 16 * 32 bytes using ymm0-ymm15 and prefetch the next 8 * 64 = 32 bytes
...
; Repeat the loop until there is no more block of 2^14 bytes
<+1587>:	add    rdi,0x3000
<+1594>:	add    rsi,0x3000
<+1601>:	dec    r10
<+1604>:	jne    0x7ffff7f27600 <__memmove_avx_unaligned_erms+1280>
; Here there is less than 2^14 left
; Check if adjusted size % (2^14) <= 128
<+1610>:	sfence
<+1613>:	cmp    edx,0x80
